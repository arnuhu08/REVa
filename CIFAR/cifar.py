...
#Adversarial = ['BIM', 'FGSM', 'PGD', 'RFGSM', 'UMIFGSM']
  ...
  def __getitem__(self, i):
    ...
    if self.no_jsd:
      ...
      im_tuple = (self.preprocess(x), aug(x, self.preprocess),
                  aug(x, self.preprocess), self.preprocess(x_adv))
      return im_tuple, y
...
def train(net, train_loader, optimizer, scheduler):
    ...
    for i, (images, targets) in enumerate(train_loader):
        ...
        aug1_images = aug1_images.cuda()
        aug2_images = aug2_images.cuda()
        ...
        # Forward pass and loss calculation for corrupted images
        ...
        if i % 100 == 0:
            print(f'Train Loss {loss_ema:.3f}')
...
def main():
  ...
  if args.dataset == 'cifar10':
    train_data = datasets.CIFAR10(
        './data/cifar', train=True, transform = train_transform, download=True) #change it to the train transforms after visualizing the plots
    # train_data_adv = datasets.CIFAR10(
    #     './data/cifar', train=True, transform=train_transform, download=True)
    adv_data = np.load('cifar10_adversarial_dataset.npy')
    ...
    train_data = datasets.CIFAR100(
        './data/cifar', train=True, transform=train_transform, download=True)
    adv_data = np.load('cifar100_adversarial_dataset.npy')
  ...
  if args.evaluate:
    ...
    print('Mean Adversarial Error: {:.3f}'.format(100 - 100. * test_a_acc))
    return
