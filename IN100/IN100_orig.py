...
# ==============================================================================
"""Main script to launch Standard training on ImageNet-100.
...
def main():
  ...
  train_transform = transforms.Compose(
      [transforms.RandomResizedCrop(224, antialias=True),
       transforms.RandomHorizontalFlip(), preprocess])
  ...
  # net = ResNet_Model(args.model, num_classes)
  ...
  log_path = os.path.join(args.save,
                          'imagenetIN100_{}_training_log.csv'.format(args.model))
  ...
  for epoch in range(start_epoch, args.epochs):
    ...
    save_path = os.path.join(args.save, 'IN100checkpoint.pth.tar')
    torch.save(checkpoint, save_path)
    if is_best:
      shutil.copyfile(save_path, os.path.join(args.save, 'IN100model_best.pth.tar'))
